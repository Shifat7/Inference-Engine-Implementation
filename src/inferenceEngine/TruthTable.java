...
public class TruthTable extends Algorithm {
	...
	public boolean CheckFacts( )
	{
		...
		for ( int i = 0; i < rowNums; i++ )
		{
			...
			if ( formulaResult[ i ] )
			{
				for ( int j = 0; j < literalIndex.length; j++ )
				{
					...
					{
						if ( (grid[i][literalIndex[j][0]]) &&
								(grid[i][literalIndex[j][1]]) &&
								(!grid[i][entailed[j]]) )
						{
							formulaResult[ i ] = false;
						}
		...
		for ( int i = 0; i < rowNums; i++ )
		{
			...
			if ( !queryResult[i] && formulaResult[i])
			{
				return false;
			}
		}
	...
	public void getVariables()
	{
		...
		for (HornClause clause : clauses) {
			for (int j = 0; j < clause.literalCount(); j++) {
				// add literals from left of the entailment
				variables.add(clause.getLiteralsAtIndex(j));
			}
			// add literals from right of the entailment
			variables.add(clause.getEntailedLiteral());
		}
		...
		Set<String> hs = new HashSet<>(variables);
		variables.clear();
		variables.addAll(hs);
